...
async function Class_Page(req, res) {
  ...
  return res.render("StaffPage/class/index", { data: { class: Class , subject:Subject_id} });
}
function Get_Create_Class(req, res)
{
    return res.render("StaffPage/class/create" , {data:{subject: req.params.subject_id}})
}
async function Post_Create_Class(req, res)
{
  ...
  New_Class.save((err)=>{
    ...
    return res.redirect("/staff/Faculty/Subject/"+req.params.subject_id+"/Class/")
  })
}
async function Get_Update_Class(req, res)
{
  ...
  let Class = await Models.ClassModel.findById({ _id: class_id })
    return res.render('StaffPage/class/edit', {data:{class: Class, subject: req.params.subject_id}})
}
function Post_Update_Class(req, res)
{
  ...
  Models.ClassModel.findByIdAndUpdate({_id: classId}, {Update_at: date, Class_ID: class_id, Class_name:class_name}).exec((err)=>{
      if(err) console.log(err)
      return res.redirect("/staff/Faculty/Subject/"+req.params.subject_id+"/Class")
  })
}
...
async function Get_Class_Detail(req, res)
{
  ...
  return res.render('StaffPage/class/detail', {data: {Class:Class, Subject:Subject}})
...
function Get_Exercise(req, res)
{
  ...
  Models.ExerciseModel.findById({Class_ID: class_id}).exec((exercise)=>{
    return res.render('StaffPage/class/exercise', {data:{exercise:exercise}})
  })
}
...
function List_Account(req, res)
{
  ...
  Models.UserModel.find({User_role: role_id}).exec((err, account)=>{
    return res.render('StaffPage/account/listAccount', {data:{account:account, role:role_id}})
  })
}
module.exports = {
