...
function Faculty_Page(req, res)
{
    Models.FacultyModel.find({}).exec((err, faculty)=>{
        if(err) return console.log(err)
         return res.render('StaffPage/Faculty/index', {data:{faculty:faculty}})
    })
}
...
function Staff_Profile(req, res)
{
    ...
    Models.UserModel.findOne({_id: staff_id}).exec((err, staff)=>{
        res.render('StaffPage/detail', {data:{staff:staff}})
    })
}
module.exports = {
